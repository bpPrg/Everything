...
typedef struct {
        ...
        char        outfile[1024];  //outfile - filepath for the output FITS file for this galaxy
...
int main(int argc, char *argv[]){
        ...
        char        buffer[1024], buffer2[1024],buf[1024];   //buffers for reading in strings
        ...
        if((nx & (4096-1)) != 0 || (ny & (4096-1)) != 0){
                fprintf(stderr,"Error: %li is not a multiple of 4096.\n",(nx & (4096-1))!=0 ? nx : ny);
                exit(1);
        }
        ...
        for(ngal = 0; ngal < ngalaxies; ngal++){
                ...
                galaxies[ngal].file = calloc(256,sizeof(char));
                if(galaxies[ngal].file == NULL){
                        fprintf(stderr, "Error allocating memory for galaxy list.");
                        exit(1);
                }
                //strcpy(galaxies[ngal].file,"!");
		...
		// OVERWRITE the outfile
                strcpy(galaxies[ngal].outfile,"!");
                //strcat(buf,buffer2);
                strcat(galaxies[ngal].outfile, buffer2);

                //printf("might have a problem %s %i\n",galaxies[ngal].outfile,ngal);
        ...
        for(gal = 0; gal < ngalaxies; gal++){
                ...
                //printf("\n\n---------------------------------------------------------------------------\n\n");
                //fprintf(stdout,"Distorting galaxy %li.\n", gal);
		...
		// Added May 13 2017
		//fprintf(" The galaxy list is %s  \n", galaxies[gal].file);

		// Check shape of galaxy list and fitsfile image
                if(naxes[0] != galaxies[gal].nx || naxes[1] != galaxies[gal].ny){
                        fprintf(stderr,"Error: galaxy %li is the wrong size. The galaxy list has shape (%li, %li) but the fits image has shape (%li, %li).\n", gal, galaxies[gal].nx, galaxies[gal].ny, naxes[0], naxes[1]);
                        exit(1);
                }
                //printf("Passed the if(naxes[0] != galaxies[gal].nx statement");
                //fprintf(stdout,"Galaxy %i has size as follows. The galaxy list has shape (%i, %i) and the fits image has shape (%i, %i).\n", gal, galaxies[gal].nx, galaxies[gal].ny, naxes[0], naxes[1]);

                //printf("\n now will allocate memory\n");                //allocate enough memory for the image
                ...
                if(outimage == NULL){
                        fprintf(stderr,"Error allocating memory for output image %li.\n", gal);
                        exit(1);
                }
                //fprintf(stdout,"onaxes: (%li, %li)\n", onaxes[0], onaxes[1]);

                //fprintf(stdout,"Making output image %li: %s\n", gal, galaxies[gal].outfile);
		//              Making output image 0: !jedisim_out/out1/distorted_0/distorted_0.fits


                fits_create_file(&outfptr, galaxies[gal].outfile, &status);
