 ...
 *
 * Debug:
 * gcc -O3 -o jeditransform jeditransform.c  -lm -lcfitsio; ./jeditransform catalog.txt dislist.txt
 *
 * Depends     : 1. jedisim_out/bulge_disk_f8/bdf8_0.fits
 *               2. jedisim_out/out0/trial1_catalog.txt
 ...
 *
 * WARNING: If I take output file simdatabase/bulge_disk_f8/bulge_disk_f8_0.fits
 *          it fails but if I take bdf8_0.fits it succeed!
 *
 * CHANGES: I made the code overwrite existing output files (Jun 01, 2017).
 *
...
#define MAG0 26.78212

/*magnitude zero point for f814w filter HST ACS/WFC camera
http://www.stsci.edu/hst/acs/analysis/zeropoints

MAG0=-2.5*lg(PHOTFLAM)+PHOTZPT

PHOTFLAM are 7.8624958E-20, 7.0331775E-20, and 2.2132778E-20 for the 3 filters;
and PHOTZPT is -21.1 for all 3 filters.
...
int main(int argc, char *argv[]){
	...
	// Edited by Bhishan Poudel on date Jun 01, 2017
	// Overwrite existing output transformed stamps.
	// e.g. jedisim_out/out1/stamp_0/stamp_0.fits.gz
	// ==> !jedisim_out/out1/stamp_0/stamp_0.fits.gz
	char outfile_overwrite[1024];
	strcpy(outfile_overwrite,"!");
	strcat(outfile_overwrite, galaxies[g].stamp1);


        //create and write output image
        fits_create_file(&outfptr, outfile_overwrite, &status);
