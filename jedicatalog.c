 ...
 *
 * Depends     : 1. physics_settings/configb.sh
 *               2. physics_settings/lens.txt
 *               3. config_ouptput_folder     # e.g. jedisim_out/out0/
 *               4. simdatabase/radius_db     # r50 radius database
 *               5. simdatabase/red_db        # redshift database
 *               6. simdatabase/scaled_bulge/scaled_bulge0_to_100.fits
 *               7. Header keywords:   MAG, MAG0, PIXSCALE, RADIUS in fitsfiles.
 *
 * Depends: physics_settings: configb.sh, lens.txt
 *          jedisim_out/ ==> out0
 *          simdatabase/ ==> radius_db, red_db
 *          simdatabase/scaled_bulge_f8/f814w_scaled_bulge0.fits to 200 .fits.
 *
 * Creates: jedisim_out/out0/scaled_bulge/trial1_: catalog.txt, convolvedlist.txt, distortedlist.txt
 *
 *
 * Inputs      :  config.sh,lens.txt, config_ouptput_folder,
 *                radius_database, redshift_database, input_fitsfiles
 *
 * Outputs     : 1. config_output_folder/catalog.txt
 ...
 *               (simdatabase/scaled_bulge_f8/f814w_scaled_bulge0_to_201.fits)
 *
 *               It also reads other parameters from following files:
 *                   1. config.sh
 *                   2. lens.txt
 *                   3. simdatabase/radius_db/20_29.dat     # r50 radius database
 *                   4. simdatabase/red_db/19_99.dat        # redshift database
 *
 *               Then, in the end it creates three catalog_and_list textfiles:
 *                   1. config_output_folder/catalog.txt # jedisim_out/out0/scaled_bulge/trial1_catalog.txt
 *                   2. config_output_folder/convolvedlist.txt
 *                   3. config_output_folder/distortedlist.txt
 *
 *              And, these catalog list will be read by jeditransform.
 *
 ...
 *
 * * Note: example of a row of  the catalog.txt
 *
 ...
 * new_mag  = 26.170000
 * new_r50  = 0.131700
 * stamp1   = jedisim_out/out0/scaled_bulge/stamp_0/stamp_0.fits.gz
 * stamp2   = jedisim_out/out0/scaled_bulge/distorted_0/distorted_0.fits
 *
 *
 * Runtime: 1 sec
 *
...
int main(int argc, char *argv[]){
    ...
    //get magnitude and radius of each of the source galaxies with cfitsio
    {
        ...
        for(nimage = 0; nimage < num_source_images; nimage++){
            if(source_images[nimage].name == NULL){
                fprintf(stderr,"Error: image file path %li is null.", nimage);
                exit(1);
            }
            char temp[1024];
    ...
    //read in the radius and redshift databases
    {
        ...
        for(mag_bin=0; mag_bin <= (max_mag-min_mag); mag_bin++){
            ...
            sprintf(radius_filename,  "%s%i.dat",radius_db_folder, mag_bin+min_mag); // from config.sh radius_db_folder="simdatabase/radius_db/"
            sprintf(redshift_filename,"%s%i.dat",red_db_folder,    mag_bin+min_mag); // e.g. simdatabase/red_db/22.dat

            //import radius bin
            radiusfp = fopen(radius_filename,"r"); // e.g. simdatabase/radius_db/22.dat
            ...
            //fprintf(stdout,"Allocated radius db bin %i of size %li.\n", mag_bin, nradius);
            rewind(radiusfp);
            nradius = 0;
            while(fgets(buffer, 1024, radiusfp) != NULL){
            ...
            redfp = fopen(redshift_filename,"r"); // e.g. simdatabase/red_db/22.dat
            ...
            //fprintf(stdout,"Allocated red db bin %i of size %li.\n", mag_bin, nred);
            rewind(redfp);
            nred = 0;
            while(fgets(buffer, 1024, redfp) != NULL){
    ...
    for(g = 0; g < ngalaxies; g++){
        ...
        //if there's no valid combination, throw an error
    ...
    fprintf(stdout,"Success! Catalog made with %li entries.\n", ngalaxies);
    return 0;
}
